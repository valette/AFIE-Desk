...
desk.FileSystem.traverse( dir, function ( file, callback ) {
    ...
    if ( index >= 0 ) {
        ...
        let func = fileName.split( '.')[ 0 ].slice( index );
        obj.function = func;
        functionalMaps[ obj.id + func ] = obj;
    ...
    desk.Actions.execute( {
        ...
        for ( let line of res.stdout.split( '\n' ) ) {
            ...
            if ( !line.includes( 'Image Dimensions') ) continue;

            for ( let d of line.match( /\d+/g ) ) {

                if ( parseFloat( d ) < 2 ) planar = true;
    ...
    Object.keys( anatFiles ).forEach( function ( anatFile ) {
        ...
        file.boxes = Object.keys( maps ).map( function ( functionName ) {
            ...
            var box = new qx.ui.form.CheckBox( maps[ functionName ].function );
            ...
            box.addListener( 'changeValue', function ( e ) {
                ...
                if ( e.getData() ) {
                    ...
                    helper = new ThresholdHelper( maps[ functionName ].file , {
    ...
    var button = new qx.ui.form.Button( 'loop' );
    list.add( button );
